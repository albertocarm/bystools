...
paste_js <- "
$(function(){
  var dz = document.getElementById('km2_dropzone');
  if(!dz) return;
  var fi = dz.querySelector('input[type=file]');
  var nameEl = dz.querySelector('.km2-dz-filename');
  function setName(t){ if(nameEl) nameEl.textContent = t || ''; }

  // Click / keyboard anywhere on the box opens the native file picker (browse).
  dz.addEventListener('click', function(e){ if(fi && e.target !== fi) fi.click(); });
  dz.addEventListener('keydown', function(e){
    if((e.key === 'Enter' || e.key === ' ') && fi){ e.preventDefault(); fi.click(); }
  });
  if(fi) fi.addEventListener('change', function(){
    setName(fi.files && fi.files.length ? fi.files[0].name : '');
  });

  // Drag & drop: hand the dropped file to the Shiny input so it uploads normally
  // (works for images and datasets alike).
  ...
  dz.addEventListener('drop', function(e){
    ...
    if(files && files.length && fi){
      try { var dt = new DataTransfer(); dt.items.add(files[0]); fi.files = dt.files; } catch(err){}
      fi.dispatchEvent(new Event('change', {bubbles: true}));
    }
  });

  // Paste an image from the clipboard (Ctrl+V / Cmd+V), unless typing in a field.
  ...
  bslib::page_sidebar(
    ...
    sidebar = bslib::sidebar(
      ...
      shiny::conditionalPanel(
        ...
        shiny::div(id = "km2_dropzone", class = "km2-dropzone", tabindex = "0",
                   role = "button", `aria-label` = "Upload, drag and drop, or paste an image",
          shiny::div(class = "km2-dz-inner",
            shiny::icon("cloud-arrow-up", class = "km2-dz-icon"),
            shiny::div(class = "km2-dz-title", "Drag & drop, paste, or browse"),
            shiny::div(class = "km2-dz-sub", "Image (PNG / JPG) or dataset (.csv .xlsx .rds .rda) · paste with Ctrl+V / ⌘V"),
            shiny::span(class = "km2-dz-or", "OR"),
            shiny::span(class = "km2-dz-btn", "Browse File")
          ),
          # The real Shiny upload control, visually hidden but fully functional:
          # clicking the box or dropping a file drives this input.
          shiny::div(class = "km2-dz-fileinput",
            shiny::fileInput("img_upload", label = NULL,
                             accept = c("image/png", "image/jpeg", ".jpg", ".png", ".rds", ".rda", ".xlsx", ".csv"))
          ),
          shiny::div(class = "km2-dz-filename")
        ),

        # ---- IMAGE PRE-PROCESSING PATHS (image mode only) ----
        shiny::conditionalPanel(
          condition = "output.is_image_mode == true",
          shiny::tags$label("Clean the plot (optional)", class="form-label", style="font-weight: 700;"),
          # PATH A: automatic engine (our Python digitizer via reticulate)
          shiny::actionButton("run_auto_preprocess", "Auto-clean + numbers at risk (beta)", class = "btn-auto w-100 mb-2", icon = shiny::icon("wand-magic-sparkles")),
          # PATH B: any vision LLM
          shiny::actionButton("open_llm_modal", "Preprocess with an LLM", class = "btn-llm w-100 mb-1", icon = shiny::icon("robot")),
          shiny::helpText(shiny::HTML("If the automatic engine fails, use an <b>LLM</b>, clean it in <b>Paint</b>, or just digitize/rectify by hand. None of these steps is mandatory.")),
          shiny::hr()
        ),

        # Example datasets
        shiny::div(style="text-align: center; margin-top: -2px; margin-bottom: 10px; color: var(--bm-muted); font-weight: 700; font-size: 0.85rem;", "- OR -"),
        ...
        # STEP 2: IMPORT
        shiny::actionButton("open_import_data", "2. Import Data (Text/LLM)", class = "btn-info w-100 mb-3", icon = shiny::icon("file-import")),

        shiny::div(
          class = "km2-config-box",
          ...
          # STEP 3 (manual route only). Not needed after Auto-clean: step 4 digitizes
          # the cleaned image automatically.
          shiny::actionButton("run_manual_dig", "3. Digitize (manual route)", class="btn-primary w-100 mt-2", icon=shiny::icon("mouse-pointer")),
          shiny::helpText("Only for the manual route. After Auto-clean, skip straight to step 4.")
        ),
...
server <- function(input, output, session) {
  ...
  kmdig_script_path <- function() {
    p <- system.file("python", "kmdig.py", package = "bystools")
    if (nzchar(p) && file.exists(p)) return(p)
    cand <- c(here::here("inst", "python", "kmdig.py"), file.path("inst", "python", "kmdig.py"))
  ...
  shiny::observeEvent(input$run_auto_preprocess, {
    ...
    tryCatch({
      ...
      # Run kmdig.py in a separate process with the env's own python.
      out <- suppressWarnings(system2(
        py, args = shQuote(c(script, vals$original_img_path, prefix)),
      ...
      msg <- paste0(
        ...
        if (n_rows >= 2) ". Numbers-at-risk loaded - check the grid; empty/red cells need fixing from the original on the right."
        else if (n_rows == 1) ". Only one numbers-at-risk row read - fill the red cells from the original on the right."
        else ". Numbers-at-risk not read reliably - fill the red cells from the original on the right."
      )
  ...
  # Shared by the manual button (step 3) and the automatic path (called from step 4
  # when no curve data exists yet), so Auto-clean never needs a separate digitize.
  ...
  run_core_analysis <- function() {
    ...
    tryCatch({
      ...
      if (is.null(vals$mode) || vals$mode != "dataset") {
        ...
        # now. This makes step 3 unnecessary when Auto-clean (or any prepared image)
        # was used -- the user can go straight from Auto-clean to step 4.
        if (is.null(vals$manual_raw_data)) {
        ...
        for (cid in unique(nrisk_all$curve)) {
          ...
          if (nrow(nr) >= 2 && nrow(km) > 0) {
            # reconstruct with midpoint interpolation
