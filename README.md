...
1. **Data Reconstruction (IPD):**
   * Automated digitization of Kaplan-Meier plots and Number at Risk tables using image processing (`magick`).
   * Reconstruction of patient-level data from digitized coordinates.
   * Instant validation with Cox models (`survival`) and survival plots.
...
You can install the development version of **bystools** directly from GitHub. It is recommended to use the following settings to force the installation and prevent package upgrade conflicts:
